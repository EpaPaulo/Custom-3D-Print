...
export async function putDesign(design, maskPng, previewPng) {
  const id = newId();
  const record = { id, createdAt: new Date().toISOString(), ...design };
  await writeAtomic(path.join(designDir(), `${id}-mask.png`), maskPng);
  if (previewPng) {
...
export async function getDesignFile(id, kind) {
  if (!isValidId(id)) return null;
  const suffix = kind === 'preview' ? '-preview.png' : '-mask.png';
  try {
    return await fs.readFile(path.join(designDir(), `${id}${suffix}`));
  } catch (err) {
    if (err.code === 'ENOENT') return null;
    throw err;
  }
